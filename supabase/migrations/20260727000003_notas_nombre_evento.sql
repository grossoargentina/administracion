-- Presupuestos: notas internas y nombre opcional del evento (ej: "Cumpleaños de 15 de Sofía")
ALTER TABLE presupuestos ADD COLUMN IF NOT EXISTS nombre_evento text;
ALTER TABLE presupuestos ADD COLUMN IF NOT EXISTS notas text;

-- Eventos: mismo nombre opcional, se copia del presupuesto al confirmar
ALTER TABLE eventos ADD COLUMN IF NOT EXISTS nombre_evento text;

-- v_eventos_base/v_pipeline/v_eventos tienen lista explícita de columnas: si no se
-- agrega acá también, la app nunca ve nombre_evento aunque la tabla lo tenga
-- (mismo problema ya visto con monto_base_ars/incluye_iva/pago_diferido)
...
SELECT id, codigo, estado, cliente_id, cliente_nombre, tipo_evento, venue, total_ars,
    ...
    saldo_cobrado, fecha_sena, fecha_saldo, motivo_perdida, presupuesto_url, contrato_url,
    notas, created_at, updated_at, fecha_armado, hora_armado, fecha_desarme, hora_desarme,
    seguro_enviado, salon_id, estado_cobro, fecha_evento, horario, horarios_evento, nombre_evento
FROM v_eventos_base;
